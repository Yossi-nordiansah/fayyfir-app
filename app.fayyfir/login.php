...
if ($_SERVER["REQUEST_METHOD"] == "POST") {  
  ...
  if ($result1->num_rows === 1) {  
    ...
    if (password_verify($password, $user["password"])) {  
      // Cek apakah masih aktif di device lain  
      if (!empty($user["session_token"]) && !empty($user["token_expiry"]) && $now < strtotime($user["token_expiry"])) {  
      ...
      $token_expiry = date("Y-m-d H:i:s", $now + (60 * 60));  
  
      $update_stmt = $conn1->prepare("UPDATE users SET session_token = ?, token_expiry = ?, is_online = 1 WHERE id = ?");  
...
$_SESSION["session_token"] = $token;  
  ...
  if ($result2->num_rows === 1) {  
    ...
    if (password_verify($password, $user["password"])) {  
      ...
      $update_stmt = $conn2->prepare("UPDATE users SET session_token = ?, token_expiry = ?, is_online = 1 WHERE id = ?");  
      $update_stmt->bind_param("ssi", $token, $token_expiry, $user["id"]);  
      $update_stmt->execute();  
      ...
      $_SESSION["user_id"] = $user["id"];  
      $_SESSION["user_name"] = $user["name"];  
      $_SESSION["email"] = $identity;  // ini bisa email atau phone, sesuai input login  
      $_SESSION["role_id"] = $user["role_id"];  
      $_SESSION["region"] = $user["region_name"];  
      $_SESSION["session_token"] = $token;  
  
// Redirect  
if ($user["role_id"] == "1" && $user["region_name"] == "Gaharu") {  
    header("Location: abdul-hadi/bahan-baku");  
} else {  
    header("Location: index");  
}  
exit();  
