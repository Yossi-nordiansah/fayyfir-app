...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  $credit_raw   = $_POST["credit"] ?? "";
  $credit       = (int) str_replace(".", "", $credit_raw); // hapus titik ribuan

  if (!$date || !$description || $credit <= 0) {
    ...
    $stmt->bind_param("sisi", $date, $user_id, $description, $credit);
    if ($stmt->execute()) {
...
<main class="pt-24 px-6 pb-32 max-w-xl mx-auto">
  ...
  <form method="POST" class="space-y-6 bg-white shadow rounded-lg p-6">
    ...
    <div>
      ...
      <input type="text" name="credit" class="w-full px-3 py-2 border border-gray-300 rounded-md" oninput="formatRupiah(this)">
    </div>
    <div>
