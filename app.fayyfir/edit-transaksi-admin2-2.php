...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if ($stmt->execute()) {
    // Cek apakah sudah ada catatan deposit sebelumnya
  // $cek_deposit = $conn->prepare("SELECT id FROM deposits_supplier WHERE supplier_id = ? AND description = 'Pengisian Kontainer' AND DATE(deposit_date) = DATE(?) LIMIT 1");
//   $cek_deposit->bind_param("is", $supplier_id, $date);
//   $cek_deposit->execute();
//   $result_deposit = $cek_deposit->get_result();
//   $existing_deposit = $result_deposit->fetch_assoc();
//   
//   $created_by = $_SESSION["user_id"];
//   
//   if ($existing_deposit) {
//     // Jika ada, update deposit
//     $update_deposit = $conn->prepare("UPDATE deposits_supplier SET credit = ?, updated_at = NOW() WHERE id = ?");
//     $update_deposit->bind_param("ii", $total_price, $existing_deposit['id']);
//     $update_deposit->execute();
//     $update_deposit->close();
//   } else {
//     // Jika belum ada, buat baru
//     $desc = "Pengisian Kontainer";
//     $insert_deposit = $conn->prepare("INSERT INTO deposits_supplier (supplier_id, deposit_date, description, credit, created_at, updated_at) VALUES (?, ?, ?, ?, NOW(), NOW())");
//     $insert_deposit->bind_param("issi", $supplier_id, $date, $desc, $total_price);
