...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if (!$nama || !$status || !$nilai) {
    ...
    $error = "Nilai modal harus berupa angka tanpa titik atau koma.";
  } else {
    $stmt = $conn->prepare(
      "INSERT INTO assets (name, status, value, created_by) VALUES (?, ?, ?, ?)"
    );
    $stmt->bind_param("ssdi", $nama, $status, $nilai, $_SESSION["user_id"]);

    if ($stmt->execute()) {
      header("Location: modal-dan-aset");
      exit();
    } else {
      echo "Gagal menyimpan aset.";
    }
  }
}
?>
...
<head>
  ...
  <title>Tambah Aset - Fayyfir</title>
...
<body class="bg-gray-100 text-gray-800 min-h-screen">
  <header class="bg-gray-900 text-white py-4 px-6 fixed top-0 left-0 right-0 z-40">
    <div class="flex justify-between items-center">
      ...
      <h1 class="text-lg font-semibold">Tambah Aset</h1>
    </div>
  </header>
