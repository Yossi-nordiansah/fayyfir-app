...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if ($date && $credit > 0) {
    ...
    $stmt->bind_param("issi", $id, $date, $description, $credit);
    if ($stmt->execute()) {
...
<header class="bg-gray-900 text-white py-4 px-6 fixed top-0 left-0 right-0 z-40">
  <div class="flex justify-between items-center">
    ...
    <h1 class="text-lg font-semibold">Refund DP Supplier</h1>
  </div>
</header>
...
<main class="pt-24 px-6 pb-32 max-w-xl mx-auto">
  ...
  <form method="POST" class="space-y-6 bg-white shadow rounded-lg p-6">
    ...
    <div>
      ...
      <input type="datetime-local" name="deposit_date" value="<?= date('Y-m-d\TH:i') ?>" class="w-full px-3 py-2 border border-gray-300 rounded-md">
    </div>
    <div>
      <label class="block text-sm font-medium">Jumlah Refund (Rp)</label>
      <input type="text" name="credit" min="1000" step="1000" class="w-full px-3 py-2 border border-gray-300 rounded-md" oninput="formatRupiah(this)" />
    </div>
    <div>
      ...
      <select name="description" id="descriptionSelect" class="w-full px-3 py-2 border border-gray-300 rounded-md bg-white" onchange="toggleCustomDesc()">
        <option value="Refund DP">Refund DP</option>
