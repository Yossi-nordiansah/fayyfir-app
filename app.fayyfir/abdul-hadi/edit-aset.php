...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if (!$nama || !$status || !$nilai) {
    $error = "Semua field wajib diisi.";
  } elseif (!is_numeric($nilai)) {
    $error = "Nilai aset harus berupa angka tanpa titik atau koma.";
  } else {
    $stmt = $conn->prepare(
      "UPDATE assets SET name = ?, status = ?, value = ? WHERE id = ?"
    );
    $stmt->bind_param("ssdi", $nama, $status, $nilai, $id);
...
<body class="bg-gray-100 text-gray-800 min-h-screen">
  <header class="bg-gray-900 text-white py-4 px-6 fixed top-0 left-0 right-0 z-40">
    <div class="flex justify-between items-center">
      ...
      <h1 class="text-lg font-semibold">Edit Aset</h1>
    </div>
  </header>
    ...
    <form action="" method="POST" class="bg-white p-6 rounded shadow space-y-4">
      <div>
        ...
        <input type="text" id="nama" name="nama"
          ...
          value="<?= htmlspecialchars($aset["name"]) ?>" />
      </div>
      <div>
        <label for="status" class="block text-sm font-medium">Status</label>
        <select id="status" name="status" class="w-full border border-gray-300 px-3 py-2 rounded mt-1 bg-white">
          <option value="aktif" <?= $aset["status"] === "aktif" ? "selected" : "" ?>>Aktif</option>
          <option value="terjual" <?= $aset["status"] === "terjual" ? "selected" : "" ?>>Terjual</option>
          <option value="rusak" <?= $aset["status"] === "rusak" ? "selected" : "" ?>>Rusak</option>
        </select>
      </div>
      <div>
        <label for="nilai" class="block text-sm font-medium">Nilai Aset (Rp)</label>
        <input type="text" id="nilai" name="nilai"
          placeholder="Contoh: 5000000"
          class="w-full border border-gray-300 px-3 py-2 rounded mt-1"
          oninput="formatRupiah(this)"
          value="<?= number_format($aset["value"], 0, ",", ".") ?>" />
      </div>
