...
// Ambil daftar produk stok untuk form pemesanan
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full text-sm divide-y divide-gray-200">
          ...
          <tbody class="divide-y divide-gray-200">
                ...
                <tr class="<?= $rowClass ?>">
                  ...
                  <td class="px-4 py-2 text-center whitespace-nowrap">
                    <div class="flex justify-center items-center gap-2">
                      ...
                      <!-- Tombol Edit -->
                      <a href="transaksi-rincian-edit.php?invoice=<?= urlencode($h['invoice_number']) ?>"
                        class="text-yellow-500 hover:text-yellow-700"
                        title="Edit Transaksi">
                        <span class="material-symbols-outlined">edit</span>
                      </a>

                      <button type="button" class="text-red-600 hover:text-red-800 delete-invoice" data-invoice="<?= htmlspecialchars($h['invoice_number']) ?>" title="Hapus Transaksi">
                        <span class="material-symbols-outlined">delete</span>
                      </button>

                      <!-- Preview Invoice PDF -->
                      <a href="transaksi-invoice.php?invoice=<?= urlencode($h['invoice_number']) ?>"
                      ...
                      <a href="transaksi-invoice.php?invoice=<?= urlencode($h['invoice_number']) ?>&download=1"
            ...
            <?php else: ?>
              <tr>
                <td colspan="7" class="px-4 py-2 text-center text-gray-500">Belum ada pembelian.</td>
              </tr>
            <?php endif; ?>
          </tbody>
        </table>
  ...
  <script>
    ...
    document.addEventListener("input", (e) => {
      if (e.target.classList.contains("qty")) {
        ...
        if (qty > stock) {
          e.target.value = stock.toLocaleString("id-ID", {
    ...
    document.addEventListener("click", async (e) => {
      if (e.target.closest(".view-invoice")) {
        ...
        try {
          ...
          const oldPreviewBtn = footer.querySelector("#previewPdfBtn");
          if (oldPreviewBtn) oldPreviewBtn.remove();
          const oldDownloadBtn = footer.querySelector("#downloadPdfBtn");
          if (oldDownloadBtn) oldDownloadBtn.remove();

          if (invoiceEl && invoiceEl.value) {
            const currentInv = encodeURIComponent(invoiceEl.value);

            const btnPreview = document.createElement("a");
            btnPreview.id = "previewPdfBtn";
    ...
    // Konfirmasi + Redirect Penghaousan Data per Invoice
    document.addEventListener("click", function(e) {
      ...
      window.location.href = "transaksi-delete.php?invoice=" + encodeURIComponent(invoice);
    });
  </script>
</body>
