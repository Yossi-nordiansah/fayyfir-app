...
========================= */
if ($container_region && $container_region !== 'Gaharu') {
  $stmt = $conn->prepare("
  ...
  <main class="pt-24 px-6 pb-32 max-w-xl mx-auto">
    <form class="space-y-6 bg-white shadow rounded-lg p-6" method="POST">
      ...
      <div>
        ...
        <input type="datetime-local" name="tanggal" class="mt-1 w-full px-3 py-2 border border-gray-300 rounded-md" value="<?= date("Y-m-d\TH:i") ?>" />
      </div>
