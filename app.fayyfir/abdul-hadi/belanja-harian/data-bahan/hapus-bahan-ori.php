...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["konfirmasi_hapus"])) {  
  $stmt = $conn->prepare("DELETE FROM bb_bahan_master WHERE id = ?");  
      ...
      <div class="bg-gray-50 rounded-xl p-5 border border-gray-200">  
        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">  
          <div>  
            ...
            <dd class="font-medium text-gray-900"><?= htmlspecialchars($bahan['nama_bahan']) ?></dd>  
          </div>  
          <div>  
            <dt class="text-gray-500">Satuan</dt>  
            <dd class="font-medium text-gray-900"><?= htmlspecialchars($bahan['satuan']) ?></dd>  
          </div>  
          <div>  
            <dt class="text-gray-500">Keterangan</dt>  
            <dd class="font-medium text-gray-900"><?= htmlspecialchars($bahan['keterangan'] ?: '-') ?></dd>  
          </div>  
        </dl>  
      </div>  
