...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["konfirmasi_hapus"])) {
  ...
  if ($delete->execute()) {
    ...
    // ==============================
    // HAPUS DATA TAHAP TERKAIT
    // ==============================
    // Hapus juga tahapan master yang terkait (untuk menghindari redundant data)
    // Jika sudah ada histori produksi, query ini akan gagal secara otomatis karena constraint FK,
    // hal ini benar agar histori produksi tidak rusak.
    $stmt_tahap = $conn->prepare("DELETE FROM bb_proses_master WHERE id_bahan = ?");
    $stmt_tahap->bind_param("i", $id);
    $stmt_tahap->execute();
