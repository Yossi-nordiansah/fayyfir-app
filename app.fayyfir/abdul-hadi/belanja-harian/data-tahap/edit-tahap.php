...
// Ambil data lama
$stmt = $conn->prepare("SELECT * FROM bb_proses_master WHERE id = ?");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Cek apakah urutan sudah ada untuk bahan ini (kecuali id ini sendiri)
    $check = $conn->prepare("SELECT id FROM bb_proses_master WHERE id_bahan = ? AND urutan_tahap = ? AND id != ?");
    $check->bind_param("iii", $id_bahan, $urutan_tahap, $id);
    $check->execute();
    ...
    if ($res->num_rows > 0) {
        ...
        $stmt = $conn->prepare("UPDATE bb_proses_master SET id_bahan = ?, nama_proses = ?, urutan_tahap = ? WHERE id = ?");
        $stmt->bind_param("isii", $id_bahan, $nama_proses, $urutan_tahap, $id);
...
<main class="lg:ml-64 bg-gray-50 min-h-screen p-4 sm:p-6 lg:p-8">
    <div class="max-w-2xl mx-auto bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="p-6">
            <h2 class="text-xl font-semibold text-gray-800 mb-6">Edit Tahapan Proses</h2>
            ...
            <form action="" method="POST">
                <div class="mb-4">
                    ...
                    <select class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" 
                        ...
                        <?php while($b = $bahan_result->fetch_assoc()): ?>
                            <option value="<?= $b['id'] ?>" <?= ($data['id_bahan'] == $b['id']) ? 'selected' : '' ?>>
                ...
                <div class="mb-4">
                    ...
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" 
                           id="nama_proses" name="nama_proses" type="text" value="<?= htmlspecialchars($data['nama_proses']) ?>" required>
                </div>
                <div class="mb-6">
                    ...
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" 
                           id="urutan_tahap" name="urutan_tahap" type="number" value="<?= htmlspecialchars($data['urutan_tahap']) ?>" required>
                </div>
