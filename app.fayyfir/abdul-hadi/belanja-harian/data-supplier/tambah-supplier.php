...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  $name = trim($_POST["nama_supplier"]);
  $kontak = trim($_POST["kontak"]);
  $alamat = trim($_POST["alamat"]);
  $catatan = trim($_POST["catatan"]);

  if ($name !== "" && $kontak !== "") {
    $stmt = $conn->prepare("INSERT INTO bb_supplier (nama_supplier, kontak, alamat, catatan, created_at) VALUES (?, ?, ?, ?, NOW())");
    $stmt->bind_param("ssss", $name, $kontak, $alamat, $catatan);
    if ($stmt->execute()) {
      $success = "✅ Supplier baru berhasil ditambahkan.";
    } else {
      $error = "Terjadi kesalahan: " . $conn->error;
    }
    $stmt->close();
  } else {
    $error = "Nama supplier dan kontak wajib diisi.";
  }
}

$activeMenu = "suppliers";
$activeModule = "Tambah Supplier";
...
<main class="lg:ml-64 bg-gray-50 min-h-screen px-4 py-6 sm:px-6 lg:px-8">
  ...
  <div class="flex flex-col sm:flex-row justify-between sm:items-center mb-8">
    ...
    <h1 class="mt-4 sm:mt-0 text-2xl font-semibold text-gray-900 tracking-tight">Tambah Supplier Baru</h1>
  </div>
    ...
    <form method="POST" class="p-6 sm:p-8">
      <div class="mb-6 border-b pb-4">
        ...
        <p class="text-gray-500 mt-1 text-sm leading-relaxed">
          Lengkapi informasi supplier di bawah ini dengan teliti untuk memastikan data produksi akurat.
        </p>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
        <!-- Nama Supplier -->
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">Nama Supplier
          ...
          <input type="text" name="nama_supplier" required
            ...
            placeholder="Misal: CV Coklat Indah">
        </div>
      ...
      <div class="mb-4">
        ...
        <textarea type="text" name="alamat" rows="3"
          class="w-full border border-gray-300 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-yellow-400 focus:border-yellow-400 outline-none transition"
          placeholder="Contoh: Jl. Coklat No.2, Surabaya"></textarea>
      </div>
      
      <!-- Catatan -->
      <div class="mb-8">
        <label class="block text-sm font-medium text-gray-700 mb-2">Keterangan (Opsional)</label>
        <textarea type="text" rows="3" name="catatan"
          class="w-full border border-gray-300 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-yellow-400 focus:border-yellow-400 outline-none transition"
          placeholder="Contoh: Minimal pembelian 15kg"></textarea>
      </div>
