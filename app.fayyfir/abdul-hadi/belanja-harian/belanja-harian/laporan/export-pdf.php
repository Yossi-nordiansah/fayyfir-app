...
switch($type) {
    ...
    case 'penyusutan':
        // Ambil semua tahapan proses
        $stages_master = [];
        $res_stages = $conn->query("SELECT id, nama_proses, urutan_tahap FROM bb_proses_master ORDER BY urutan_tahap ASC");
        ...
        foreach($stages_master as $stage) {
            $id_m = (int)$stage['id'];
            $name = $stage['nama_proses'];
            $stage_cols .= ", (SELECT IFNULL(SUM(penyusutan),0) FROM bb_proses_detail WHERE id_pembelian = pa.id AND id_proses_master = $id_m) AS `$name (Kg)`";
        }

        $sql = "
        SELECT 
            pa.tanggal_pembelian AS `Tanggal`,
            pa.kode_batch AS `Batch`
            $stage_cols,
            (SELECT IFNULL(SUM(penyusutan),0) FROM bb_proses_detail WHERE id_pembelian = pa.id) AS `Total Susut (Kg)`
        FROM bb_pembelian_awal pa
        " . ($id ? "WHERE pa.kode_batch = '$id'" : "") . "
