...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["kode_batch"])) {
  ...
  if ($bahan_id && $supplier_id && $berat > 0 && $harga_kg > 0 && !empty($tanggal_pembelian)) {
    $stmt = $conn->prepare("
      ...
      (kode_batch, tanggal_pembelian, id_bahan, id_supplier, berat_awal, harga_per_kg, total_modal, catatan, status, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'proses', NOW())
    ");
    $stmt->bind_param("ssiiddds", $kode_batch_post, $tanggal_pembelian, $bahan_id, $supplier_id, $berat, $harga_kg, $total, $catatan);
    if ($stmt->execute()) {
      echo "<script>
...
$activeModule = "Input Pembelian Awal";
