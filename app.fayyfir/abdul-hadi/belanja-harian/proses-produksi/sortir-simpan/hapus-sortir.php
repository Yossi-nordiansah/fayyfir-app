...
if ($id === 0) {
  header("Location: list-sortir.php");
  exit();
}
...
if ($confirm === 'yes') {
  // Proses hapus data sortir
  $stmt = $conn->prepare("DELETE FROM bb_proses_sortir WHERE id = ?");
  $stmt->bind_param("i", $id);
  if ($stmt->execute()) {
    // Redirect ke halaman list
    header("Location: list-sortir.php?msg=hapus-success");
    exit();
  } else {
    die("Gagal menghapus data sortir.");
  }
} else {
  // Ambil data sortir untuk ditampilkan pada konfirmasi
  $stmt = $conn->prepare("
    SELECT ps.id, pa.kode_batch, b.nama_bahan
  ...
  if ($result->num_rows === 0) {
    header("Location: list-sortir.php?msg=not-found");
    exit();
  }
...
$activeModule = "Hapus Data Proses";
    ...
    <div class="p-6 sm:p-8">
      <div class="mb-5 border-b pb-4">
        ...
        <p class="text-gray-500 mt-1 text-sm leading-relaxed">
          Penghapusan data proses ini bersifat permanen dan tidak dapat dibatalkan.
      ...
      <div class="mt-8 justify-between flex flex-col sm:flex-row gap-3">
        <a href="hapus-sortir.php?id=<?= $id ?>&confirm=yes"
