...
try {
    ...
    $is_group_closing = ($target_id_pembelian > 0);
    ...
    if ($is_group_closing && $target_id_pembelian > 0) {
        ...
        $conn->query("UPDATE bb_pembelian_awal SET status = 'selesai_siap_jual' WHERE id = $target_id_pembelian");
    } else {
        // ===== CLOSING UNTUK SINGLE BATCH TERTIMBANG =====
        $sqlRaw = "
            SELECT 
                SUM(pd.berat_masuk) as total_raw_intake,
                SUM(pd.berat_masuk * pa.harga_per_kg) as total_raw_cost
            FROM bb_proses_detail pd
