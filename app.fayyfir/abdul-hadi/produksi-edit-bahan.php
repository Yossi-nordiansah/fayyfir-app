...
$stmt = $conn->prepare("  
  SELECT pm.material_id, pm.quantity_used, pm.unit_price, m.name, pm.unit_id, u.symbol
...
$stmt->execute();  
$stmt->bind_result($material_id, $existing_quantity, $unit_price, $material_name, $current_unit_id, $stock_unit_symbol);  
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["quantity"], $_POST["unit_price"])) {  
  $new_quantity = floatval(str_replace('.', '', $_POST['quantity']));  
  $new_unit_id = isset($_POST["unit_id"]) ? (int) $_POST["unit_id"] : $current_unit_id;  
  ...
  if ($new_quantity > 0 && $new_quantity <= $stok_asli) {  
    ...
    $stmt = $conn->prepare("UPDATE production_materials SET quantity_used = ?, unit_price = ?, unit_id = ? WHERE id = ?");  
    $stmt->bind_param("ddii", $new_quantity, $new_unit_price, $new_unit_id, $pm_id);  
    $stmt->execute();  
    $stmt->close();  
...
<div>  
  ...
  <input type="text" name="quantity" id="quantity" inputmode="decimal" value="<?= number_format($existing_quantity, 0, ',', '.') ?>" required class="w-full px-3 py-2 border rounded-lg" />  
</div>  
...
// Initial call to set stock text correctly based on existing input
const initialQty = unformat(qtyInput.value);
stockText.innerText = formatter.format(realStock - initialQty);
