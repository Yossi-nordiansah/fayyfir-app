...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $fee_per_kg = $transaksi['fee_per_kg']; // Ambil dari database
  $harga_asli = $harga_per_kg - $fee_per_kg;
  ...
  $stmt = $conn->prepare("UPDATE transactions SET transaction_date=?, driver_name=?, driver_phone=?, vehicle_plate=?, sack_count=?, weight_kg=?, price_per_kg=$harga_asli, grand_total=?, notes=?, supplier_id=?, weight_input_by_role=?, price_input_by_role=? WHERE id=?");
  $stmt->bind_param("ssssiiisiiii", $date, $driver_name, $driver_phone, $vehicle_plate, $sack_count, $weight, $grand_total, $notes, $supplier_id, $weight_input_by_role, $price_input_by_role, $transaction_id);
...
$total_per_kg = $transaksi['price_per_kg'] + $transaksi['fee_per_kg'];
...
<body class="bg-gray-100 text-gray-800 min-h-screen"><header class="bg-gray-900 text-white py-4 px-6 fixed top-0 left-0 right-0 z-40">
  ...
  <input type="text" id="supplierBaru" name="supplierBaru" placeholder="Masukkan Nama Petani Baru" class="mt-2 w-full px-3 py-2 border border-yellow-400 rounded-md hidden text-sm" />
</div>
...
<div>
  ...
  <input type="text" id="harga_per_kg_display" class="mt-1 w-full px-3 py-2 border border-gray-300 rounded-md" value="<?= number_format($transaksi['price_per_kg'] + $transaksi['fee_per_kg'], 0, ',', '.') ?>" />
  <input type="hidden" id="harga_per_kg" name="harga_per_kg" value="<?= $transaksi['price_per_kg'] + $transaksi['fee_per_kg'] ?>" />
</div>

<div hidden>
  <input type="text" id="fee_per_kg_display" value="<?= number_format($transaksi['fee_per_kg'], 0, ',', '.') ?>" />
...
function updateHarga() {
  ...
  const total = beratVal * hargaVal;

  grandTotal.value = total;
  grandTotalDisplay.value = formatter.format(total);
