...
<body class="bg-gray-100 text-gray-800 min-h-screen">
  ...
  <main class="pt-24 px-6 pb-32 max-w-xl mx-auto">
    <form class="space-y-6 bg-white shadow rounded-lg p-6" method="POST">
      ...
      $is_price_hidden = ($session_role_id == 1 && ($transaksi["price_input_by_role"] == 3 || $transaksi["price_input_by_role"] == 2));
      ?>
  ...
  <script>
    ...
    function updateTotalHarga() {
      ...
      const hargaVal = hargaDisplay ? parseRibuan(hargaDisplay.value) : 0;
      const totalVal = beratVal * hargaVal;

      if (total) total.value = totalVal;
      if (totalDisplay) totalDisplay.value = formatter.format(totalVal);
      updateGrandTotal();
    }
    ...
    function updateGrandTotal() {
      const totalHargaVal = total ? (parseInt(total.value) || 0) : 0;
