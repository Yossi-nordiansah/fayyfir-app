...
<main class="pt-24 px-4 pb-32 max-w-7xl mx-auto space-y-10">
  ...
  <section class="grid grid-cols-2 md:grid-cols-4 gap-4">
    ...
    <div class="bg-white shadow rounded-xl p-4">
      ...
      <p class="text-lg font-bold">Rp <?= number_format($total_production, 0, ',', '.') ?></p>
    </div>
    <div class="bg-white shadow rounded-xl p-4">
    ...
    <table class="table-auto w-full text-sm">
      <thead class="bg-gray-100">
        <tr>
          ...
          <th class="px-3 py-2">Selisih (gram)</th>
      ...
      <tbody id="productionTable">
        <?php foreach ($productions as $row): ?>
          <?php 
            ...
            $total_weight = $row['total_weight'] ?? 0;
            $total_output = $row['total_output'] ?? 0;
            $selisih = $total_weight - $total_output;
            
            $total_biaya_kg = $total_biaya / ($total_output / 1000);
          ?>
          <tr class="border-b hover:bg-gray-50">
            <td class="px-3 py-2 whitespace-nowrap"><?= htmlspecialchars($row['production_date']) ?></td>
            <td class="px-3 py-2 whitespace-nowrap font-mono"><?= htmlspecialchars($row['production_number']) ?></td>
            <td class="px-3 py-2 whitespace-nowrap"><?= htmlspecialchars($row['status']) ?></td>
            <td class="px-3 py-2 whitespace-nowrap text-right"><?= number_format($row['total_weight'], 0, ',', '.') ?></td>
            <td class="px-3 py-2 whitespace-nowrap text-right"><?= number_format($row['total_output'], 0, ',', '.') ?></td>
            <td class="px-3 py-2 whitespace-nowrap text-right"><?= number_format($selisih, 0, ',', '.') ?></td>
            <td class="px-3 py-2 whitespace-nowrap text-right">Rp <?= number_format($total_biaya_kg, 0, ',', '.') ?></td>
            <td class="px-3 py-2 whitespace-nowrap text-right">Rp <?= number_format($total_biaya, 0, ',', '.') ?></td>
            <td class="px-3 py-2 whitespace-nowrap text-right">Rp <?= number_format($row['price_weight'] ?? 0, 0, ',', '.') ?></td>
            <td class="px-3 py-2 whitespace-nowrap text-right">Rp <?= number_format($row['fix_price'] ?? 0, 0, ',', '.') ?></td>
