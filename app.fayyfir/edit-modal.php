...
// Ambil data lama
$stmt = $conn->prepare("SELECT * FROM modal_log WHERE id = ?");
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if (!$tanggal || !$deskripsi || !$nilai) {
    ...
    $stmt = $conn->prepare(
      "UPDATE modal_log SET date = ?, description = ?, amount = ? WHERE id = ?"
    );
    $stmt->bind_param("ssdi", $tanggal, $deskripsi, $nilai, $id);
...
<head>
  ...
  <title>Edit Modal - Fayyfir</title>
...
<body class="bg-gray-100 text-gray-800 min-h-screen">
  <header class="bg-gray-900 text-white py-4 px-6 fixed top-0 left-0 right-0 z-40">
    <div class="flex justify-between items-center">
      ...
      <h1 class="text-lg font-semibold">Edit Modal</h1>
    </div>
  </header>
    ...
    <form action="" method="POST" class="bg-white p-6 rounded shadow space-y-4">
      <div>
        ...
        <input type="date" id="tanggal" name="tanggal"
          ...
          value="<?= htmlspecialchars($modal['date']) ?>" />
      </div>
      <div>
        <label for="deskripsi" class="block text-sm font-medium">Deskripsi</label>
        <input type="text" id="deskripsi" name="deskripsi"
          placeholder="Contoh: Modal tambahan dari investor"
          class="w-full border border-gray-300 px-3 py-2 rounded mt-1"
          value="<?= htmlspecialchars($modal['description']) ?>" />
      </div>
      <div>
        ...
        <input type="text" id="nilai" name="nilai"
          ...
          value="<?= number_format($modal['amount'], 0, ',', '.') ?>" />
      </div>
